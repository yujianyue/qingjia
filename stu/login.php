    ...
    switch ($act) {
        case 'login':
            ...
            if ($user['type'] !== 'student') {
                json_result(1, '您不是学生用户，请前往相应入口登录');
            }
            ...
            json_result(0, '登录成功', ['url' => 'stu.php?do=qjia']);
            break;
...
<div class="login-container">
    <div class="login-title">学生用户登录</div>
    
    <form id="loginForm" class="login-form">
        <div class="form-group">
            <label class="form-label">学号</label>
            <input type="text" name="username" class="form-control" placeholder="请输入学号">
        </div>
    ...
    <div class="login-links" style="margin-top: 20px; text-align: center;">
        ...
        <a href="zhi.php?do=login">指导员登录</a>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('loginBtn').addEventListener('click', function() {
            ...
            // 发送登录请求
            ajaxRequest('stu.php?do=login&act=login', formData, function(res) {
                if (res.code === 0) {
